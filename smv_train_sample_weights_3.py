...
'''
features = []
with open ('/Users/chongshao-mikasa/Data/video_data_in_txt/video1_features.dat','rb') as csvfile:
...
'''  
indices = []
with open ('/Users/chongshao-mikasa/Data/video_data_in_txt/video1_indices.dat','rb') as csvfile2:
...
for i in xrange(len(Xo)):
  if Xo[i][-1] < mid_threshold and neg_c < 100: 
    neg_c += 1
    xx.append(Xo[i])
    yy.append(0)
  elif Xo[i][-1] > mid_threshold and pos_c < 100:
    pos_c += 1
    xx.append(Xo[i])
    yy.append(1)
  if neg_c == pos_c == 100:
...
for i in xrange(len(Xo)):
  if Xo[i][-1] < mid_threshold and neg_c < 100:
    ...
    xxw.append(Xo[i])
    yyw.append(0)
    weights.append(mid_threshold / Xo[i][-1])
    dw.append(10*mid_threshold / Xo[i][-1])
  elif Xo[i][-1] > mid_threshold and pos_c < 100:
    pos_c += 1
    xxw.append(Xo[i])
    yyw.append(1)
    weights.append(Xo[i][-1] / mid_threshold)
...
plt.scatter(proj_xo[:,0], proj_xo[:,1], cmap=pl.cm.Paired, s= dw)
...
img_ww_file = open("img_ww.txt", "w")
img_ww_file.write(img_ww.strip())
img_ww_file.close()

img_wnw_file = open("img_wnw.txt","w")
